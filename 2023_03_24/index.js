...
console.log(req.session.message);
    res.render('gallery', { data: galleryData, message: req.session.message });
...
app.post('/gallery/:id', auth, async (req, res) => {
    ...
    } else if(!galleryData[req.params.id].ratings.find(entry => entry.userId === ratingData.userId)){

        galleryData[req.params.id].ratings.push(ratingData);
    } else {
        const yourRating = galleryData[req.params.id].ratings.filter(entry => entry.userId === ratingData.userId).rating;
        req.session.message = `You have already rated this photo. Your rating was ${yourRating}`;
        // return res.render('gallery', { data: galleryData, message: req.session.message});
